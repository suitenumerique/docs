...
in
{
  options.services.docs = {
    ...
    port = mkOption {
      ...
      default = 8000;
      description = ''
        Port used by gunicorn to listen.
      '';
    };
  ...
  config = mkIf cfg.enable {
    services.docs.config = {
      ...
      DJANGO_ALLOWED_HOSTS = mkDefault "localhost,127.0.0.1"; 
    })
    ...
    systemd.services.docs = {
      ...
      script = ''
        ${optionalString (cfg.secretKeyPath != null) "export DJANGO_SECRET_KEY=$(cat ${cfg.secretKeyPath})"}
        ${optionalString (
        ...
        ) "export Y_PROVIDER_API_KEY=$(cat ${cfg.collaborationServer.serverSecretPath})"}
    ...
    services.redis.servers.docs = mkIf cfg.redis.createLocally {
      enable = true;
    };
