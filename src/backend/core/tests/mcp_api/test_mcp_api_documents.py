...
def test_mcp_documents_read_own_document(
    mock_convert, user_token, resource_server_backend, user_specific_sub, yhub_content
):
    ...
    # the content is read from the collaboration server, which owns it
    yhub_content.assert_called_once()
    assert mock_convert.call_args.args[0] == factories.YDOC_HELLO_WORLD_UPDATE
...
@patch.object(YHubService, "create_ydoc")
@patch("core.services.converter_services.YdocConverter.convert")
def test_mcp_documents_create_allowed_for_editor(
    ...
    mock_convert.return_value = factories.YDOC_HELLO_WORLD_UPDATE
    parent = factories.DocumentFactory(link_reach=models.LinkReachChoices.RESTRICTED)
    factories.UserDocumentAccessFactory(
    ...
    # the converted content seeds the document on the collaboration server
    mock_create_ydoc.assert_called_once_with(
        document, factories.YDOC_HELLO_WORLD_UPDATE
    )
...
def test_mcp_documents_create_root_creates_owner_access(
    ...
    mock_create_ydoc.assert_called_once()


@patch.object(YHubService, "create_ydoc")
@patch("core.services.converter_services.YdocConverter.convert")
def test_mcp_documents_create_without_content(
    mock_convert,
    mock_create_ydoc,
    user_token,
    resource_server_backend,
    ...
    """An empty document has nothing to convert nor to seed."""
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {user_token}")
    response = client.post("/api/v1.0/mcp/documents", {"title": "Doc", "content": ""})
...
@patch.object(YHubService, "create_ydoc")
@patch("core.services.converter_services.YdocConverter.convert")
def test_mcp_documents_create_collaboration_server_down(
    mock_convert,
    mock_create_ydoc,
    user_token,
    resource_server_backend,
    ...
    """A document whose content could not be seeded must not be left behind."""
    mock_convert.return_value = factories.YDOC_HELLO_WORLD_UPDATE
    mock_create_ydoc.side_effect = ServiceUnavailableError("down")

    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {user_token}")
    response = client.post(
        "/api/v1.0/mcp/documents", {"title": "Doc", "content": "Hello"}
    )
