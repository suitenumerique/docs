...
var syncProtocol__namespace =
  /*#__PURE__*/ _interopNamespaceDefault(syncProtocol);
var authProtocol__namespace =
  /*#__PURE__*/ _interopNamespaceDefault(authProtocol);
var awarenessProtocol__namespace =
  /*#__PURE__*/ _interopNamespaceDefault(awarenessProtocol);
...
messageHandlers[messageSync] = (
  ...
  if (
    emitSynced &&
    syncMessageType === syncProtocol__namespace.messageYjsSyncStep2 &&
    !provider.synced
  ) {
...
const readMessage = (provider, buf, emitSynced) => {
  console.error('coucou READ MESSAGE');
...
const setupWS = (provider) => {
  ...
  if (provider.shouldConnect && provider.ws === null) {
    ...
    // console.error('coucou ASSIGNIN ONMESSAGE');

    websocket.addEventListener('message', (event) => {
      console.error('coucou NEW MESSAGE E');
    ...
    //   }
    // };
    console.error('------- HAS BEEN SET');

    websocket.onerror = (event) => {
      //   console.error('coucou ON ERROR');

      provider.emit('connection-error', [event, provider]);
    };
    websocket.onclose = (event) => {
      //   console.error('coucou ON CLOSE');
      closeWebsocketConnection(provider, websocket, event);
    };
    websocket.onopen = () => {
      //   console.error('coucou ON OPEN');
      provider.wsLastMessageReceived = time__namespace.getUnixTime();
      provider.wsconnecting = false;
      ...
      if (provider.awareness.getLocalState() !== null) {
        ...
        encoding__namespace.writeVarUint(
...
const broadcastMessage = (provider, buf) => {
  //   console.error('coucou BROADCAST');
...
class WebsocketProvider extends observable.ObservableV2 {
  ...
  constructor(
    ...
    // console.error('coucou CONSTRUCTOR');
    ...
    this._bcSubscriber = (data, origin) => {
      console.error('coucou BC SUBSCRIBER');
      ...
      if (origin !== this) {
        ...
        if (encoding__namespace.length(encoder) > 1) {
          bc__namespace.publish(
            this.bcChannel,
            encoding__namespace.toUint8Array(encoder),
            this,
          );
        }
      }
    ...
    this._updateHandler = (update, origin) => {
      //   console.error('coucou UPDATE HANDLER');
  ...
  connectBc() {
    ...
    bc__namespace.publish(
      this.bcChannel,
      encoding__namespace.toUint8Array(encoderSync),
      this,
    );
    ...
    bc__namespace.publish(
      this.bcChannel,
      encoding__namespace.toUint8Array(encoderState),
      this,
    );
