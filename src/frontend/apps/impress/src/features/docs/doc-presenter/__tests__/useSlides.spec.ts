...
const image = (id?: string): TestBlock => ({
  ...(id ? { id } : {}),
  type: 'image',
  props: { url: 'x' },
});
...
describe('hasDividerBlock', () => {
  test('returns false when no divider is present', () => {
    expect(hasDividerBlock([para('a'), para('b')])).toBe(false);
  });

  test('returns true when a divider is present', () => {
    expect(hasDividerBlock([para('a'), divider(), para('b')])).toBe(true);
  });

  test('returns true when a divider is nested', () => {
    expect(
      hasDividerBlock([quote('intro'), para('a'), divider([para('b')])]),
    ).toBe(true);
  });
});
